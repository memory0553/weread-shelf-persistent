...
import type { ShelfSyncResponse } from './models';
...
class ApiV2Manager {
	...
	async getShelf(): Promise<ShelfSyncResponse | undefined> {
		return this.callAgent<ShelfSyncResponse>('/shelf/sync');
	}
	...
	async getCurrentUser(): Promise<Record<string, unknown> | undefined> {
		// 先从书架拿一个 bookId
		const shelf = await this.getShelf();
