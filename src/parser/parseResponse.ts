...
export const parseShelfMetadata = (shelfBook: ShelfBookItem, noteBook?: any): Metadata => {
	const notebookMetadata = noteBook ? parseMetadata(noteBook) : undefined;
	const bookId = String(shelfBook.bookId);
	const recentTimestamp =
		shelfBook.readUpdateTime ?? shelfBook.updateTime ?? noteBook?.sort ?? 0;
	const cover = (shelfBook.cover ?? notebookMetadata?.cover ?? '').replace('/s_', '/t7_');
	const author = (shelfBook.author ?? notebookMetadata?.author ?? '未知作者').replace(
		/\[(.*?)\]/g,
		'【$1】'
	);
	...
	return {
		bookId,
		author,
		title: shelfBook.title ?? notebookMetadata?.title ?? '未命名',
		url: shelfBook.deepLink ?? notebookMetadata?.url ?? '',
		pcUrl: getPcUrl(bookId),
		cover,
		bookType: shelfBook.type ?? notebookMetadata?.bookType ?? 0,
		publishTime: shelfBook.publishTime ?? notebookMetadata?.publishTime ?? '',
		noteCount: notebookMetadata?.noteCount ?? 0,
		reviewCount: notebookMetadata?.reviewCount ?? 0,
		lastReadDate:
			recentTimestamp > 0
				? window.moment(recentTimestamp * 1000).format('YYYY-MM-DD')
				: notebookMetadata?.lastReadDate ?? ''
	};
};
