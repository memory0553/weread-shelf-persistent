...
export const WEREAD_BOOKSHELF_VIEW_ID = 'weread-shelf-persistent-bookshelf-view';
...
export class WereadBookshelfView extends ItemView {
	...
	private renderActionIcons(book: BookshelfBook, container: HTMLElement): void {
		if (book.remoteExists && !book.hasLocalFile && book.noteCount + book.reviewCount > 0) {
	...
	private renderBadges(book: BookshelfBook, container: HTMLElement): void {
		...
		if (book.noteCount + book.reviewCount === 0) {
			labels.push('暂无笔记');
		}
