...
export default class SyncNotebooks {
	...
	async syncBookById(bookId: string) {
		...
		if (!currentBookMeta) {
			new Notice('该书不在当前微信读书书架中，或暂无可同步的划线和想法');
			return;
		}
	...
	private async getALlMetadata() {
		const [notebookResp, shelfBooks] = await Promise.all([
			this.apiManager.getNotebooksWithRetry(),
			this.shelfRepository.getShelfBooks()
		]);
		const shelfBookIds = new Set(shelfBooks.map((book) => String(book.bookId)));
		return notebookResp
			.map((noteBook) => parseMetadata(noteBook))
			.filter(
				(metaData) =>
					shelfBookIds.has(String(metaData.bookId)) &&
					metaData.noteCount + metaData.reviewCount > 0
			);
	}
