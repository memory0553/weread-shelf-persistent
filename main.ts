...
import ShelfRepository from './src/shelfRepository';
...
export default class WereadPlugin extends Plugin {
	...
	private initializePlugin() {
		...
		const shelfRepository = new ShelfRepository(this, apiRouter);
		this.syncNotebooks = new SyncNotebooks(
			fileManager,
			apiRouter,
			this.app.vault,
			shelfRepository
		);
		this.bookshelfService = new WereadBookshelfService(
			fileManager,
			apiRouter,
			shelfRepository
		);
	...
	private async syncCookiesToPartition(): Promise<void> {
		...
		try {
			...
			const WEREAD_PARTITION = 'persist:weread-shelf-persistent-browser';
