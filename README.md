...
这是基于 Obsidian Weread Plugin 2.2.0 的本地独立版本，插件 ID 为
`weread-shelf-persistent`，不会被官方 `obsidian-weread-plugin` 的更新覆盖。

本地版本的核心差异：

- 书架页面使用官方 Agent API `/shelf/sync`，展示微信读书当前真实书架。
- `/user/notebooks` 只用于获取划线和想法数量，不再充当书架数据源。
- 笔记同步仅处理“当前书架中且划线或想法数量大于 0”的书。
- 最近一次成功获取的书架保存为插件目录下的 `bookshelf-cache.json`；接口暂时失败时使用缓存。
- 不展示已移出微信读书书架的本地独有笔记。
